...
// questao 4
int cortar_baralho(int pos, pilha *p) {
    int i, num_elems;
    pilha aux;
    Carta carta;

    pilha_criar(&aux, p->tamanho);

    num_elems = pilha_numero_elementos(*p);

    if(num_elems < pos || pos <= 0)
        return POSICAO_INVALIDA;
    
    for(i = pos; i <= num_elems; i++) {
        carta = p->elementos[i];
        pilha_inserir(carta, &aux);
    }
...
int main () {
    ...
    printf("\nForneça a posicao de corte: ");
    scanf("%d", &pos);

    cortar_baralho(pos, &cartas);
