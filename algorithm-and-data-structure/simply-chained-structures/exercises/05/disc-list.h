...
/*** QUESTAO 5 ***/
lista lista_cd_alternar_listas(lista listaUm, lista listaDois) {
    no_enc *noUm, *noDois;
    lista listaIntercalada;

    noUm = listaUm;
    noDois =listaDois;
    listaIntercalada = NULL;

    // insere enquanto nenhum chegou ao fim
    while(noUm != NULL && noDois != NULL) {
        lista_cd_inserir(noUm->elemento, &listaIntercalada);
        lista_cd_inserir(noDois->elemento, &listaIntercalada);
        noUm = noUm->proximo;
        noDois = noDois->proximo;
    }

    // Se a um chegou ao fim passar a apontar pra dois, caso contrario a um ainda tem elemento
    if(noUm == NULL) {
        noUm = noDois;
    }

    // insere os elementos faltantes da lista que foi decidida no if acima
    while(noUm != NULL) {
        lista_cd_inserir(noUm->elemento, &listaIntercalada);
        noUm = noUm->proximo;
    }
